...
class SocketIoChatService implements ChatService {
  ...
  Future<void> connect({required String token, required String room}) async {
    ...
    _s!.connect();
    _s!.onConnect((_) {
      _s!.emit('join', {'room': room});
      _ctrl.add(const ChatEvent('system', content: 'Socket.IO 連線成功'));
    });
    _s!.on(
        'message',
        (data) => _ctrl.add(ChatEvent('message',
            from: data['from']?.toString(),
            content: data['text']?.toString())));
    _s!.on('like', (_) => _ctrl.add(const ChatEvent('like')));
    _s!.onDisconnect(
        (_) => _ctrl.add(const ChatEvent('system', content: '已斷線')));
  }
