...
class CartPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: items.isEmpty ? const Center(child: Text('購物車是空的')) : ListView.separated(
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          padding: const EdgeInsets.all(12),
          child: Row(children: [
            Expanded(child: Text('合計：\$${total.toStringAsFixed(2)}', style: Theme.of(context).textTheme.titleLarge)),
            FilledButton(onPressed: ()=> context.push('/checkout'), child: const Text('結帳')),
          ]),
        ),
      ),
    );
